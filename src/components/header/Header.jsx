...
const Header = () => {
  ...
  return (
    <header className="w-full h-18 fixed top-0 flex flex-row items-center justify-between px-7 sm:px-10 xl:px-30 border-b border-(--border) bg-transparent backdrop-blur-lg z-40">
      ...
      <NavBar />
      <a href="#kontakt" className="flex items-center gap-2 px-7 py-2 rounded-full bg-linear-to-r from-(--primary) to-(--secondary) text-(--button-text) font-semibold text-[13px] cursor-pointer max-[900px]:hidden">
        Kontaktiraj me <FaArrowRightLong className="text-(--button-text)" />
      </a>
      <span
        onClick={toggleMenu}
        className="text-2xl text-(--white) min-[900px]:hidden cursor-pointer"
      >
